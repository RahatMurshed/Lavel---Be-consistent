...
export default function AIInsights({ avgCompletion, streak, totalXP, bestDay, worstDay }: Props) {
  ...
  const fetchInsights = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("consistency-coach", {
        body: {
          prompt: `Provide a brief 3-4 sentence analytics summary. Avg completion: ${avgCompletion}%. Current streak: ${streak} days. Total XP: ${totalXP}. Best day: ${bestDay}. Worst day: ${worstDay}. Give actionable advice.`,
        },
      });
